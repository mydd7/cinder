...
const readPool = createPool("cinder-read");
...
ipcMain.handle("usage:transcript", (_e, source, session) =>
  readPool.run({ kind: "transcript", source: String(source || ""), session: String(session || "") }, {
    supported: true,
    messages: []
  })
);
